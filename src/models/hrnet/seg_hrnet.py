...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, path_to_config, classes, final_upsampling, **kwargs):
        config = read_config(path_to_config, "MODEL")
        extra = config['EXTRA']
...
class HighResolutionNetOCR(nn.Module):
    ...
    def __init__(self, path_to_config, classes, final_upsampling, **kwargs):
        ...
        # OCR
        self.L = nn.Conv2d(720, classes, 1)
        self.X = conv2d(720, 512, 3)
    ...
    def forward(self, x):
        ...
        if self.final_upsampling is not None and self.final_upsampling > 1:
            out = F.interpolate(out, scale_factor=self.final_upsampling, mode='bilinear', align_corners=True)
            aux_out = F.interpolate(L, scale_factor=self.final_upsampling, mode='bilinear', align_corners=True)
...
if __name__=='__main__':
    model = HighResolutionNetOCR(path_to_config='/home/danil.akhmetov/Projects/SeaIce/src/models/hrnet/hrnet_w48.yaml',
                                 classes=1, final_upsampling=4)

    print(model.__class__.__name__)
    batch = torch.zeros((4,3,512,512))
    model(batch)