...
from loop.optimizer import Lookahead
...
class Factory:
    ...
    def make_model(self, device) -> torch.nn.Module:
        ...
        if isinstance(self.params.get('weights', None), str) and len(self.params['weights']) > 0:
            state_dict = torch.load(self.params['weights'])
    ...
    def make_scheduler(optimizer, stage):
        if '.' in stage['scheduler']:
            ...
            return getattr(torch.optim.lr_scheduler, stage['scheduler'])(optimizer=optimizer,
                                                                         **stage['scheduler_params'])

    @staticmethod
    def make_loss(stage, device) -> torch.nn.Module:
        loss = pydoc.locate(stage['loss'])(**stage['loss_params'])
